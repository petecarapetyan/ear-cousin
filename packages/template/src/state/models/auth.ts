  ...
  effects(store: Store) {
    return {
      ...
      init() {
        ...
        // store.getDispatch().fixme.fixmes({}) // OR SWAP OUT TO userFixmes
        // store.getDispatch().fixme.focusDcmnt({})
      ...
      async subscribeRoles() {
        if (store.getState()?.auth?.user?.uid) {
          ...
          store.getDispatch().fixme.crudMode('r');
          // MODIFY MANUALLY
          store.getDispatch().fixme.populateFixmes();
        }
      },
    };
  },
