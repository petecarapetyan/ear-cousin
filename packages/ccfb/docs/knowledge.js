...
var knowledge = o({
    ...
    effects: (store) => ({
        async populateKnowledges() {
            ...
            if (path) {
                const knowledgeRef = ba(db, path);
                const querySnapshot = await lh(Mu(knowledgeRef));
                querySnapshot.forEach((doc) => {
                    const docData = doc.data();
                    const knowledge = docData;
                    knowledge.id = doc.id;
        ...
        async loadUpdateView(key) {
            ...
            if (path) {
                const knowledgeRef = va(db, path);
                if (knowledgeRef) {
                    const docSnap = await ch(knowledgeRef);
...
const getState = (state) => state.knowledge;
// creates methods such as `KnowledgeSelectors.someval(state)`
// which would typically return state.someval
var KnowledgeSelectors;
(function (KnowledgeSelectors) {
    KnowledgeSelectors.knowledgeFocus = createSelector([getState], (state) => state.focusDcmnt);
    KnowledgeSelectors.knowledges = createSelector([getState], (state) => state.knowledges);
    KnowledgeSelectors.crudMode = createSelector([getState], (state) => state.crudMode);
})(KnowledgeSelectors || (KnowledgeSelectors = {}));
...
let KnowledgeC = class KnowledgeC extends Connected {
    ...
    submit(form) {
        ...
        dispatch.knowledge.create(data);
    }
    render() {
        return p$1 `
      <div if-c>
...
KnowledgeC = __decorate([
    n('view-knowledge-c')
], KnowledgeC);

let KnowledgeRD = class KnowledgeRD extends Connected {
    mapState(state) {
        return {
...
], KnowledgeRD.prototype, "knowledges", void 0);
KnowledgeRD = __decorate([
    n('view-knowledge-rd')
], KnowledgeRD);

let KnowledgeU = class KnowledgeU extends Connected {
    mapState(state) {
        return {
    ...
    submit(form) {
        ...
        data["bar"] = form["bar"].value;
        data["yada"] = form["yada"].value;
        dispatch.knowledge.updateDcmnt(data);
    }
    render() {
        return p$1 `
      <div if-u>
...
__decorate([
    e({ type: Object })
], KnowledgeU.prototype, "knowledgeFocus", void 0);
KnowledgeU = __decorate([
    n('view-knowledge-u')
], KnowledgeU);
