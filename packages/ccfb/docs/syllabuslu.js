...
var syllabuslu = o({
    ...
    effects: (store) => ({
        async populateSyllabuslus() {
            ...
            if (path) {
                const syllabusluRef = ba(db, path);
                const querySnapshot = await lh(Mu(syllabusluRef));
                querySnapshot.forEach((doc) => {
                    const docData = doc.data();
                    const syllabuslu = docData;
        ...
        async delete(key) {
            ...
            if (path) {
                const syllabusluRef = va(db, path);
                if (syllabusluRef) {
                    await mh(va(db, path));
        ...
        async loadUpdateView(key) {
            ...
            if (path) {
                const syllabusluRef = va(db, path);
                if (syllabusluRef) {
                    const docSnap = await ch(syllabusluRef);
...
const getState = (state) => state.syllabuslu;
// creates methods such as `SyllabusluSelectors.someval(state)`
// which would typically return state.someval
var SyllabusluSelectors;
(function (SyllabusluSelectors) {
    SyllabusluSelectors.syllabusluFocus = createSelector([getState], (state) => state.focusDcmnt);
    SyllabusluSelectors.syllabuslus = createSelector([getState], (state) => state.syllabuslus);
    SyllabusluSelectors.crudMode = createSelector([getState], (state) => state.crudMode);
})(SyllabusluSelectors || (SyllabusluSelectors = {}));

var models = /*#__PURE__*/Object.freeze({
    __proto__: null,
    auth: auth,
    syllabuslu: syllabuslu
});
...
let SyllabusluC = class SyllabusluC extends Connected {
    ...
    submit(form) {
        ...
        dispatch.syllabuslu.create(data);
    }
    render() {
        return p$1 `
      <div if-c>
...
SyllabusluC = __decorate([
    n('view-syllabuslu-c')
], SyllabusluC);

let SyllabusluRD = class SyllabusluRD extends Connected {
    mapState(state) {
        return {
...
], SyllabusluRD.prototype, "syllabuslus", void 0);
SyllabusluRD = __decorate([
    n('view-syllabuslu-rd')
], SyllabusluRD);

let SyllabusluU = class SyllabusluU extends Connected {
    mapState(state) {
        return {
    ...
    submit(form) {
        ...
        data["bar"] = form["bar"].value;
        data["meh"] = form["meh"].value;
        dispatch.syllabuslu.updateDcmnt(data);
    }
    render() {
        return p$1 `
      <div if-u>
...
__decorate([
    e({ type: Object })
], SyllabusluU.prototype, "syllabusluFocus", void 0);
SyllabusluU = __decorate([
    n('view-syllabuslu-u')
], SyllabusluU);
...
let AppShellElement = class AppShellElement extends Connected {
    ...
    render() {
        return this.authenticated
            ? p$1 `
          <view-syllabuslu-c if-c></view-syllabuslu-c>
          <view-syllabuslu-u if-u></view-syllabuslu-u>
          <view-syllabuslu-rd if-rd></view-syllabuslu-rd>
...
//# sourceMappingURL=syllabuslu.js.map
