    ...
    effects(store) {
        return {
            ...
            init() {
                ...
                // store.getDispatch().cohort.cohorts({}) // OR SWAP OUT TO userCohorts
                // store.getDispatch().cohort.focusDcmnt({})
...
var cohort = o({
    ...
    effects: (store) => ({
        async populateCohorts() {
            ...
            if (path) {
                const cohortRef = ba(db, path);
                const querySnapshot = await lh(Mu(cohortRef));
                querySnapshot.forEach((doc) => {
                    const docData = doc.data();
                    const cohort = docData;
                    cohort.id = doc.id;
        ...
        async loadUpdateView(key) {
            ...
            if (path) {
                const cohortRef = va(db, path);
                if (cohortRef) {
                    const docSnap = await ch(cohortRef);
...
const getState = (state) => state.cohort;
// creates methods such as `CohortSelectors.someval(state)`
// which would typically return state.someval
var CohortSelectors;
(function (CohortSelectors) {
    CohortSelectors.cohortFocus = createSelector([getState], (state) => state.focusDcmnt);
    CohortSelectors.cohorts = createSelector([getState], (state) => state.cohorts);
    CohortSelectors.crudMode = createSelector([getState], (state) => state.crudMode);
})(CohortSelectors || (CohortSelectors = {}));
...
let CohortC = class CohortC extends Connected {
...
let CohortRD = class CohortRD extends Connected {
    mapState(state) {
        return {
...
], CohortRD.prototype, "cohorts", void 0);
CohortRD = __decorate([
    n('view-cohort-rd')
], CohortRD);

let CohortU = class CohortU extends Connected {
    mapState(state) {
        return {
    ...
    submit(form) {
        ...
        data["bar"] = form["bar"].value;
        data["yada"] = form["yada"].value;
        dispatch.cohort.updateDcmnt(data);
    }
    render() {
        return p$1 `
      <div if-u>
...
__decorate([
    e({ type: Object })
], CohortU.prototype, "cohortFocus", void 0);
CohortU = __decorate([
    n('view-cohort-u')
], CohortU);
...
let AppShellElement = class AppShellElement extends Connected {
    ...
    render() {
        return this.authenticated
            ? p$1 `
          <view-cohort-c if-c></view-cohort-c>
          <view-cohort-u if-u></view-cohort-u>
          <view-cohort-rd if-rd></view-cohort-rd>
