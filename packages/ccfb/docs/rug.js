    ...
    effects(store) {
        return {
            ...
            init() {
                ...
                // store.getDispatch().rug.rugs({}) // OR SWAP OUT TO userRugs
                // store.getDispatch().rug.focusDcmnt({})
            ...
            async subscribeRoles() {
                if (store.getState()?.auth?.user?.uid) {
                    ...
                    store.getDispatch().rug.crudMode('r');
                    // MODIFY MANUALLY
                    store.getDispatch().rug.populateRugs();
                }
            },
        };
    },
...
var rug = o({
    ...
    effects: (store) => ({
        async populateRugs() {
            ...
            if (path) {
                const rugRef = ba(db, path);
                const querySnapshot = await lh(Mu(rugRef));
                querySnapshot.forEach((doc) => {
                    const docData = doc.data();
                    const rug = docData;
                    rug.id = doc.id;
                    dispatch.rug.upsert(rug);
                });
            }
        },
        async create(data) {
            ...
            if (path) {
                ...
                if (rugRef) {
                    ...
                    console.error("ADDED rug ID", docRef.id);
                }
                store.getDispatch().rug.populateRugs();
                store.getDispatch().rug.crudMode('r');
        ...
        async delete(key) {
            ...
            if (path) {
                const rugRef = va(db, path);
                if (rugRef) {
                    await mh(va(db, path));
                    store.getDispatch().rug.deleteDcmnt(key);
                    window.location.reload();
                }
            }
        },
            ...
            if (path) {
                ...
                if (rugRef) {
                    ...
                    if (data && docSnap.id) {
                        ...
                        store.getDispatch().rug.focusDcmnt(rug);
                    }
                }
            }
        },
...
// see ../index.ts for where state.rug below,
// is defined using default export from above
const getState = (state) => state.rug;
// creates methods such as `RugSelectors.someval(state)`
// which would typically return state.someval
var RugSelectors;
(function (RugSelectors) {
    RugSelectors.rugFocus = createSelector([getState], (state) => state.focusDcmnt);
    RugSelectors.rugs = createSelector([getState], (state) => state.rugs);
    RugSelectors.crudMode = createSelector([getState], (state) => state.crudMode);
})(RugSelectors || (RugSelectors = {}));
...
let RugC = class RugC extends Connected {
    // Why this? Allows the form submit to fire submit(form) method
    firstUpdated() {
        const form = this.shadowRoot?.getElementById("rugForm");
        if (!!form) {
            form.onsubmit = (e) => {
                e.preventDefault();
                this.submit(form);
    ...
    submit(form) {
        ...
        data["size"] = form["size"].value;
        data["sizzle"] = form["sizzle"].value;
        dispatch.rug.create(data);
    }
    render() {
        return p$1 `
      <div if-c>
        <h3>Add Rug Document to the Collection:</h3>
        <form id="rugForm">
          <fieldset>
            <label for="source">Source:</label><br/>
            <input type="text" id="source" name="source" value="Source  ${Date.now()}" /><br />
                        <label for="size">Size:</label><br/>
            <input type="text" id="size" name="size" value="Size  ${Date.now()}" /><br />
                        <label for="sizzle">Sizzle:</label><br/>
            <input type="text" id="sizzle" name="sizzle" value="Sizzle  ${Date.now()}" /><br />
            <br />
...
let RugRD = class RugRD extends Connected {
    mapState(state) {
        return {
            rugs: RugSelectors.rugs(state),
        };
    }
    delete(key) {
        dispatch.rug.delete(key);
    }
    addView() {
        dispatch.rug.crudMode('c');
    }
    loadUpdateView(_e) {
        dispatch.rug.crudMode('u');
        dispatch.rug.loadUpdateView(_e?.target?.["id"]);
    }
    render() {
        return p$1 `
      <div if-r>
        <h3>Rugs</h3>
        <a><button type="button" @click=${() => (this.addView())}>Add Rug Document</button></a>
        <div class="table">
            ${Object.keys(this.rugs).map(key => {
            const dcmnt = this.rugs[key];
            return p$1 `
                <div class="row">
                  <div class="cell">
                  ...
                  <div class="cell">${dcmnt["source"]}</div>
                  <div class="cell">${dcmnt["size"]}</div>
                  <div class="cell">${dcmnt["sizzle"]}</div>
                </div>
              `;
        })}
        </div>
...
], RugRD.prototype, "rugs", void 0);
RugRD = __decorate([
    n('view-rug-rd')
], RugRD);

let RugU = class RugU extends Connected {
    mapState(state) {
        return {
            rugFocus: RugSelectors.rugFocus(state),
        };
    }
    ...
    submit(form) {
        let data = {};
        data["id"] = this.rugFocus.id;
        data["source"] = form["source"].value;
        data["size"] = form["size"].value;
        data["sizzle"] = form["sizzle"].value;
        dispatch.rug.updateDcmnt(data);
    }
    render() {
        return p$1 `
      <div if-u>
        <h3>Update Rug Document with ID of <em>'${this.rugFocus.id}'</em> </h3>
        <form id="rugForm">
          <fieldset>
            <label for="source">Source:</label><br/>
            <input type="text" id="source" name="source" value="${this.rugFocus.source ? this.rugFocus.source : ""}" /><br />
                        <label for="size">Size:</label><br/>
            <input type="text" id="size" name="size" value="${this.rugFocus.size ? this.rugFocus.size : ""}" /><br />
                        <label for="sizzle">Sizzle:</label><br/>
            <input type="text" id="sizzle" name="sizzle" value="${this.rugFocus.sizzle ? this.rugFocus.sizzle : ""}" /><br />
            <br />
            <input type="submit" value="Submit" />
          </fieldset>
        </form>
...
__decorate([
    e({ type: Object })
], RugU.prototype, "rugFocus", void 0);
RugU = __decorate([
    n('view-rug-u')
], RugU);
...
let AppShellElement = class AppShellElement extends Connected {
    ...
    render() {
        return this.authenticated
            ? p$1 `
          <view-rug-c if-c></view-rug-c>
          <view-rug-u if-u></view-rug-u>
          <view-rug-rd if-rd></view-rug-rd>
