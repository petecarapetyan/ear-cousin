    ...
    effects(store) {
        return {
            ...
            init() {
                ...
                // store.getDispatch().event.events({}) // OR SWAP OUT TO userEvents
                // store.getDispatch().event.focusDcmnt({})
...
var event = o({
    ...
    effects: (store) => ({
        async populateUserEvents() {
            ...
            if (path) {
                const userEventRef = ba(db, path);
                const querySnapshot = await lh(Mu(userEventRef));
                querySnapshot.forEach((doc) => {
                    const docData = doc.data();
                    const userEvent = docData;
                    userEvent.id = doc.id;
        ...
        async loadUpdateView(key) {
            ...
            if (path) {
                const userEventRef = va(db, path);
                if (userEventRef) {
                    const docSnap = await ch(userEventRef);
...
var EventSelectors;
(function (EventSelectors) {
    EventSelectors.userEventFocus = createSelector([getState], (state) => state.focusDcmnt);
    EventSelectors.userEvents = createSelector([getState], (state) => state.userEvents);
    EventSelectors.crudMode = createSelector([getState], (state) => state.crudMode);
...
let UserEventC = class UserEventC extends Connected {
...
UserEventC = __decorate([
    n('view-event-c')
], UserEventC);

let EventRD = class EventRD extends Connected {
    mapState(state) {
        return {
...
], EventRD.prototype, "userEvents", void 0);
EventRD = __decorate([
    n('view-event-rd')
], EventRD);

let EventU = class EventU extends Connected {
    mapState(state) {
        return {
    ...
    submit(form) {
        ...
        data["ackby"] = form["ackby"].value;
        data["ackdatetime"] = form["ackdatetime"].value;
        dispatch.event.updateDcmnt(data);
    }
    render() {
        return p$1 `
      <div if-u>
...
__decorate([
    e({ type: Object })
], EventU.prototype, "userEventFocus", void 0);
EventU = __decorate([
    n('view-event-u')
], EventU);
...
let AppShellElement = class AppShellElement extends Connected {
    ...
    render() {
        return this.authenticated
            ? p$1 `
          <view-event-c if-c></view-event-c>
          <view-event-u if-u></view-event-u>
          <view-event-rd if-rd></view-event-rd>
